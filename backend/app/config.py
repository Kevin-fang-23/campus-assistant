...
class Settings(BaseSettings):
    ...
    # ---- /api/qa 缓存的语义相似匹配 ----
    # 字面归一化无法识别「同一意图的不同问法」：
    #   「作业什么时候截止」 vs 「作业截止时间是什么」→ 归一化后是两个 key，
    #   会各走一次完整流程（各烧一次 LLM + embedding），而答案是同一份。
    # 开启后：精确 key 未命中时再比对该查询向量与缓存中已有 query 向量的余弦相似度，
    # 超过阈值即判定命中、直接复用上次的 AnswerOut。
    #
    #   · 0 = 关闭（完全退回字面归一化，可随时回滚）。关闭时**不计算查询向量**，
    #     因此没有任何额外 embedding 开销。
    #   · 开启时也不多花 embedding：精确 key 先查（零成本），只有未命中才计算向量，
    #     且该向量会透传给 hybrid_search 复用（见 api/qa.py）。命中省一次 LLM，
    #     未命中与改造前开销相同。
    #
    # 阈值 0.86 的实测依据（backend/eval/run_cache_threshold_eval.py，
    # 22 对标注样本 = 10 条改写 + 12 条相近但异义，方法论与混合权重调参一致）：
    #   · 上界（异义对最高相似度）= 0.797；下界（改写对最低相似度）= 0.498
    #     → 两者区间重叠，余弦单判据**无法完全分离**，这是该判据的已知边界；
    #   · 零误配前提下最高命中档 = 6/10，覆盖阈值 0.80~0.86（平台区）。
    #     取平台区**最高**值 0.86：观测收益相同，但距最高负样本余量最大（0.063）。
    #   · 收益：改写问法的复用率 字面 1/10 → 语义 6/10。
    #   · 取舍依据：漏命中只是多花一次 LLM，误配是用户拿到另一个问题的答案
    #     且无从察觉 —— 先把误配压到 0，再谈命中率。
    #
    # ⚠ 阈值**随 embedding 后端变化**，换后端必须重跑标定，不要沿用本值：
    #   同一批样本在 local_hash（256 维字符哈希）上上界 0.667 / 下界 0.136，
    #   任何阈值都只能命中 1/10（与字面归一化持平 = **零增量价值**）。
    #   local_hash 是字面哈希而非语义模型，本质识别不了改写问法；
    #   生产用 dashscope（1024 维）才有意义。启动日志会提示当前后端的实际状况。
    qa_cache_semantic_threshold: float = 0.86
    ...
    #
    # 阈值取值的实测依据（噪声样本 30 条 = 10 校外话题 + 20 校园无答案，
    # 见 RETRIEVAL_BASELINE.md 第五节「扩样后的实测」）：
    #   · T=1：拦噪声 8/30（校外 6/10，校园 2/20），真实误杀 **0/129**
    #   · T=2：拦 20/30，但误杀 9/129（7%）
    #   · T=3：拦 28/30，但误杀 24/129（18.6%）
    # T=1 的定位是「零成本的第一道滤网」：拦下大部分校外噪声、零误杀；
    # 「校园相关但语料无答案」类查询共享的是通用校园词汇（期末/宿舍/食堂/预约），
    # 词面计数在原理上无法区分，需语义级判定 —— 已列为后续方向，不靠提阈值硬扛。
    #
